...
async def on_reaction_add(reaction , user):
  if reaction.emoji == '🇬':
    ...
    embed.add_field(name = 'mv!enterme',value ='Use it like ``mv!enterme <giveaway channel>`` to enter in a giveaway running in a particular channel(Sometimes may not work)',inline = False)
  ...
  if reaction.emoji == '🇲':
    ...
    embed.add_field(name = 'mv!showme(Requires a role named Giveaways)',value ='To see how many people are taking part in giveaway(Sometimes may not work)',inline = False)
    embed.add_field(name = 'mv!pickwinner(Requires a role named Giveaways)',value ='To pick winner(Sometimes may not work)',inline = False)
  ...
  for channel in user.server.channels:
    if channel.name == 'i•-general-chat-•i' and reaction.emoji == '😑':
      ...
      role = discord.utils.get(user.server.roles, name='Server Developer')
      await client.add_roles(user, role)
      print('Added codies role in ' + (user.name))
      await client.send_message(user, embed=embed)
