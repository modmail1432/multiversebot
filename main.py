...
async def on_reaction_add(reaction , user):
  if reaction.emoji == '🇬':
    ...
    embed.add_field(name = 'mv!whoami',value ='Tells you what your username is; this is how you are saved in the file.',inline = False)
    embed.add_field(name = 'mv!real',value ='Tells you what the real username of a mentioned user is. This is how they are saved in the file.',inline = False)
    embed.add_field(name = 'mv!choose',value ='Chooses between multiple choices.',inline = False)
...
async def setreactionverify(ctx):
    ...
    for channel in author.server.channels:
        if channel.name == '★verify-for-chatting★':
            react_message = await client.send_message(channel, 'React with <a:WOW:515854429485006848> to Verify')
            reaction = '<a:WOW:515854429485006848>'
            await client.add_reaction(react_message, reaction)
